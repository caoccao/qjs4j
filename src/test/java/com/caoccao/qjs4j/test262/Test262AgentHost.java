...
final class Test262AgentHost implements AutoCloseable {
    ...
    private JSNativeFunction createAgentFunction(JSContext context, String name, int length,
    ...
    public JSObject createAgentObject(JSContext context, List<JSRuntime> realmRuntimes, Test262Agent agent) {
        ...
        agentObject.set("sleep", createAgentFunction(context, "sleep", 1, (ctx, thisArg, args) -> {
            long milliseconds = 0;
            if (args.length > 0) {
        ...
        if (agent == null) {
            agentObject.set("start", createAgentFunction(context, "start", 1, (ctx, thisArg, args) -> {
            ...
            agentObject.set("broadcast", createAgentFunction(context, "broadcast", 1, (ctx, thisArg, args) -> {
            ...
            agentObject.set("getReport", createAgentFunction(context, "getReport", 0, (ctx, thisArg, args) -> {
                String report = reports.poll();
                return report == null ? JSNull.INSTANCE : new JSString(report);
            }));
        } else {
            agentObject.set("receiveBroadcast",
                    createAgentFunction(context, "receiveBroadcast", 1, (ctx, thisArg, args) -> {
            ...
            agentObject.set("report", createAgentFunction(context, "report", 1, (ctx, thisArg, args) -> {
                String report = args.length > 0 ? JSTypeConversions.toString(ctx, args[0]).value() : "undefined";
                reports.offer(report);
                return JSUndefined.INSTANCE;
            }));
            agentObject.set("leaving", createAgentFunction(context, "leaving", 0, (ctx, thisArg, args) -> {
                agent.markLeaving();
                return JSUndefined.INSTANCE;
            }));
            agentObject.set("getReport",
                    createAgentFunction(context, "getReport", 0, (ctx, thisArg, args) -> JSNull.INSTANCE));
            agentObject.set("start", createAgentFunction(context, "start", 1, (ctx, thisArg, args) -> ctx
                    .throwTypeError("$262.agent.start is only available on the main agent")));
            agentObject.set("broadcast", createAgentFunction(context, "broadcast", 1, (ctx, thisArg, args) -> ctx
                    .throwTypeError("$262.agent.broadcast is only available on the main agent")));
        }
        return agentObject;
    }

    public JSNativeFunction createSetTimeoutFunction(JSContext context) {
        return createAgentFunction(context, "setTimeout", 2, (ctx, thisArg, args) -> {
            if (args.length < 1 || !(args[0] instanceof JSFunction callback)) {
    ...
    public void installAtomicsHelperOverrides(JSContext context) {
        ...
        agentObject.set(PropertyKey.fromString("getReportAsync"),
        ...
        agentObject.set(PropertyKey.fromString("tryYield"),
        ...
        agentObject.set(PropertyKey.fromString("trySleep"),
