 ...
 * <p>
 * {@code JSArrayBuffer}, {@code JSTypedArray}, {@code JSDataView}, the weak collections, the number formatters and the
 * RegExp bytecode used to raise {@code IllegalArgumentException}/{@code IllegalStateException}. Those carry no error
 * type, so every caller had to convert them by hand; a path that forgot reported an opaque internal error instead of
 * the specified {@code TypeError} or {@code RangeError}. They now raise {@code JSTypeErrorException},
 * {@code JSRangeErrorException} or {@code JSSyntaxErrorException}, which the interpreter turns into a catchable
 * JavaScript error of the right type on its own.
 * <p>
...
public class JSEngineErrorTypingTest extends BaseJavetTest {
    ...
    public void testNumberFormattingRangeErrors() {
        assertStringWithJavet("try { (1.5).toFixed(101) } catch (e) { 'CAUGHT ' + e.name }",
    ...
    public void testNumberFormattingStillWorks() {
        assertStringWithJavet("(1.5).toFixed(2)", "(1.5).toPrecision(3)", "(255).toString(16)");
    }

    @Test
    public void testRegExpErrorsAreSyntaxErrors() {
        assertStringWithJavet("try { new RegExp('a', 'qq') } catch (e) { 'CAUGHT ' + e.name }",
    ...
    public void testWeakCollectionKeyErrorsAreTypeErrors() {
        assertStringWithJavet("try { new WeakMap().set(1, 2) } catch (e) { 'CAUGHT ' + e.name }",
