 ...
 * <p>
 * The VM routes JavaScript exceptions through the {@code pendingException} field, but the engine also raises errors by
 * throwing Java exceptions ({@code JSException}, {@code JSVirtualMachineException}, {@code JSRangeErrorException}, ...)
 * out of an opcode handler. Those used to unwind the Java stack straight out of {@code VirtualMachine.execute()} and
 * were never offered to the script's catch handlers, so an ordinary temporal-dead-zone {@code ReferenceError} killed
 * the whole evaluation.
 * <p>
 * Each snippet below evaluates to {@code "CAUGHT <name>"}. If an error escapes the script's {@code try}, the qjs4j side
 * throws instead of producing a string and the assertion fails.
 */
public class JSEngineErrorChannelTest extends BaseJavetTest {

    @Test
    public void testArrayBufferLengthRangeErrorIsCatchable() {
        assertStringWithJavet("try { new ArrayBuffer(9007199254740991) } catch (e) { 'CAUGHT ' + e.name }",
    ...
    public void testTemporalDeadZoneConstIsCatchable() {
        assertStringWithJavet("try { tc; const tc = 1 } catch (e) { 'CAUGHT ' + e.name }", """
                (function () {
    ...
    public void testTemporalDeadZoneLetIsCatchable() {
        assertStringWithJavet("try { tz; let tz = 1 } catch (e) { 'CAUGHT ' + e.name }", """
                (function () {
    ...
    public void testTypedArrayRangeErrorIsCatchable() {
        assertStringWithJavet("try { new Int8Array(9007199254740991) } catch (e) { 'CAUGHT ' + e.name }", """
                try {
