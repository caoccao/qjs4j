 ...
 * <p>
 * The lexer has always known this: {@code café} is an identifier, {@code \u0078} <em>is</em> {@code x}, and the
 * {@code StringValue} of {@code "a\u002db"} is {@code a-b}. The module transformer did not. It re-extracted binding
 * names with {@code [A-Za-z_$][A-Za-z0-9_$]*} and read string literals with {@code substring}, so the parser accepted
 * source that the transformer then gave different semantics — a module exporting {@code café} reported no such export,
 * and {@code export const \u0078 = 1} was rejected as "not a valid identifier" for a binding the compiler had just
 * accepted.
 * <p>
 * These are the cases expressible in a single module, so V8 can be the arbiter directly. The cross-module halves —
 * escaped specifiers, escaped arbitrary export names between two files — need a module graph on disk and live in
 * {@link JSModuleNameResolutionTest}.
 */
