 ...
 * <p>
 * The line-oriented module transformer answered it by counting brackets, braces and parentheses and taking lines while
 * they were unbalanced. Balanced delimiters do not mark the end of an AssignmentExpression — {@code export default 1 +}
 * is balanced and incomplete — so a valid module became {@code let X = (0, 1 +);} and failed to parse at a position in
 * text the author never wrote. The count also had no notion of a regular expression literal, so {@code /\(/} opened a
 * depth that swallowed the statement after it, and the lexer read the {@code /} after {@code default} as division, so
 * such a module did not tokenise at all.
 * <p>
...
public class JSModuleDefaultExportExtentTest extends BaseJavetTest {
    ...
    public void testADefaultExportOfATaggedTemplateSpanningLines() {
        assertStringWithJavet("const tag = (strings) => strings.raw[0];\n" + "export default tag`first\nsecond`;\n"
                + "String('ok');");
    }
    ...
    public void testDefaultAsAPropertyNameIsStillDivision() {
        ...
        // value to compare.
        moduleMode = false;
        assertStringWithJavet("String(({ default: 8 }).default / 2);",
