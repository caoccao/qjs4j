 ...
 * <p>
 * The temporal-dead-zone error said only {@code "variable is uninitialized"} — it named no variable, so a TDZ error in
 * a large function gave nothing to go on, even though the local variable names are in the bytecode. Reading a property
 * of {@code null} said {@code "cannot read property 'foo' of null"} where V8 says
 * {@code "Cannot read properties of null (reading 'foo')"}.
 * <p>
 * The failed-delete and failed-assignment messages name the receiver, and V8's rule for that is subtler than it looks:
 * {@code Object::NoSideEffectsToMaybeString} produces {@code #<Ctor>} only while the object's {@code toString} is still
 * {@code Object.prototype.toString}, so an Array or a Date — which carry their own — fall through to
 * {@code [object Tag]} instead. Reporting {@code #<Array>} for an array was the visible symptom of not implementing
 * that test.
 */
public class JSErrorMessageParityTest extends BaseJavetTest {
    ...
    public void testNullPropertyReadMessage() {
        assertStringWithJavet("try { null.foo } catch (e) { e.message }", "try { null.length } catch (e) { e.message }",
    ...
    public void testTemporalDeadZoneMessageNamesTheVariable() {
        assertStringWithJavet("try { tz; let tz = 1 } catch (e) { e.message }",
    ...
    public void testUndefinedPropertyReadMessage() {
        assertStringWithJavet("try { undefined.foo } catch (e) { e.message }",
