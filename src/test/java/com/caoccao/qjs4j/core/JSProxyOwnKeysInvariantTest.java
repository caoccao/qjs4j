...
/**
 * The {@code ownKeys} proxy invariant check compares the trap result against the target's non-configurable own keys
 * through a {@code HashSet<PropertyKey>}.
 * <p>
 * The trap result is converted with {@code PropertyKey.fromString("0")} while the target's keys arrive as
 * {@code PropertyKey.fromIndex(0)}. Those two encodings used to be neither {@code equals} nor hash-equal, so a
 * perfectly conformant trap over a frozen array was rejected with a spurious
 * {@code TypeError: 'ownKeys' on proxy: trap result did not include '0'} — naming a key that was right there in the
 * set.
 */
