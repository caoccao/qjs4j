 ...
 * <p>
 * The dense fast path first asks whether anything on the prototype chain could observe the write. That question used to
 * be answered by a walk bounded at 1,000 links which returned "no interference" when it ran out of depth — the unsafe
 * direction. A setter 1,001 links up was therefore never called and an own element was created instead, so
 * {@code array[0] = 1} diverged from the specification purely as a function of how deep the chain was.
 * <p>
 * Every assertion runs the same source through V8 and through qjs4j, so what counts as correct here is what V8 does
 * rather than a reading of the specification. {@code ../quickjs/qjs} agrees with V8 on all of it: the setter runs and
 * no own property appears, at every depth.
 */
public class JSArrayDeepPrototypeSetTest extends BaseJavetTest {

    /**
     * Build source that puts a setter for index 0 at the far end of a prototype chain, assigns through it, and reports
     * whether the setter ran and whether an own element appeared.
     *
     * @param depth
     *            links between the array and the object holding the setter
