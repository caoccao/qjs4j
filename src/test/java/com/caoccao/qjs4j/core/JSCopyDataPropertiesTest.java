...
/**
 * Object spread ({@code {...src}}) and object rest ({@code {a, ...rest}}) must copy properties with CreateDataProperty,
 * not with {@code [[Set]]}.
 * <p>
 * ES2024 7.3.25 CopyDataProperties step 4.c.ii specifies CreateDataPropertyOrThrow — an own property definition that
 * ignores the prototype chain. Copying with {@code [[Set]]} instead let a setter or a non-writable data property on
 * {@code Object.prototype} intercept the copy: the setter ran (an unexpected side channel with prototype-pollution
 * flavour) and the property never landed on the result object.
 * <p>
 * {@code Object.assign} genuinely does use {@code [[Set]]}, so it is asserted here too — the two must stay
 * distinguishable.
 */
