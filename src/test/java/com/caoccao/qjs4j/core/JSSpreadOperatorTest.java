...
/**
 * Tests for the ECMAScript spread operator (...). The spread operator allows an iterable (like an array) to be expanded
...
public class JSSpreadOperatorTest extends BaseJavetTest {
    ...
    public void testArrayLiteralSpreadString() {
        assertStringWithJavet("JSON.stringify([...'abc'])", "JSON.stringify([1, ...'xy', 2])",
                "JSON.stringify([...''])", "JSON.stringify([...Number(123).toString()])",
                "JSON.stringify([...'🌟😀'])");
    }

    @Test
    public void testArrayLiteralSpreadWithHoles() {
        assertStringWithJavet("JSON.stringify([0, ...[1, 2], , 3])", "JSON.stringify([0, ...[1, 2], , 3])",
                "JSON.stringify([, 1, 2])", "JSON.stringify([0, , 2])", "JSON.stringify([0, 1, ,])",
                "JSON.stringify([, , 2])", "JSON.stringify([, , ,])", "JSON.stringify([...[0], , ...[1, 2], , 3])",
                "JSON.stringify([0, , ...[1, 2]])", "JSON.stringify([...[0, 1], , 2])", "JSON.stringify([0,1,,,])",
                "JSON.stringify([0,...[1,2],,])", "JSON.stringify([0,,,...[1,2]])",
                "JSON.stringify([...[0],,...[1,2],,3])", "JSON.stringify([,0,,...[1],,2,...'ab',,3,,])",
    ...
    public void testMathMaxWithSpread() {
        assertIntegerWithJavet("Math.max(...[1, 5, 3, 2, 4])");
    }

    @Test
    public void testMathMinWithSpread() {
        assertIntegerWithJavet("Math.min(...[5, 3, 1, 4, 2])");
    }
    ...
    public void testMultipleSpreadInSameArray() {
        assertStringWithJavet("JSON.stringify([...[1], ...[2], ...[3], ...[4, 5]])");
    }
    ...
    public void testSpreadWithDestructuring() {
        assertIntegerWithJavet("const [a, ...rest] = [...[1, 2, 3, 4]]; rest.length");
    }
