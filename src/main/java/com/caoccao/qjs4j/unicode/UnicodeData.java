...
/**
 * Unicode character property tables and lookups. Based on QuickJS libunicode.c implementation.
 */
public final class UnicodeData {

    // ASCII character type table (0-255)
    private static final byte[] ASCII_CTYPE_BITS = new byte[256];
    private static final int[] ID_CONTINUE_RANGES = UnicodePropertyResolver
            .resolveBinaryPropertyUncached("ID_Continue");
    ...
    /**
     * Check if character can continue an identifier. Follows ECMAScript specification (ID_Continue property).
     */
    public static boolean isIdentifierPart(int codePoint) {
        if (codePoint < 128) {
            // ASCII fast path
            return (ASCII_CTYPE_BITS[codePoint]
                    & (UNICODE_C_UPPER | UNICODE_C_LOWER | UNICODE_C_UNDER | UNICODE_C_DOLLAR | UNICODE_C_DIGIT)) != 0;
        }
    ...
    /**
     * Check if character can start an identifier. Follows ECMAScript specification (ID_Start property).
     */
    public static boolean isIdentifierStart(int codePoint) {
        if (codePoint < 128) {
            // ASCII fast path
            return (ASCII_CTYPE_BITS[codePoint]
                    & (UNICODE_C_UPPER | UNICODE_C_LOWER | UNICODE_C_UNDER | UNICODE_C_DOLLAR)) != 0;
        }
    ...
    /**
     * Check if character is a line terminator. ECMAScript recognizes: LF, CR, LS, PS
     */
    ...
    /**
     * Check if character is a whitespace character. Follows ECMAScript specification.
     */
    ...
    public static boolean isWordChar(int codePoint) {
        if (codePoint < 256) {
            return (ASCII_CTYPE_BITS[codePoint]
                    & (UNICODE_C_UPPER | UNICODE_C_LOWER | UNICODE_C_UNDER | UNICODE_C_DIGIT)) != 0;
        }
        return false;
    }
}
