...
/**
 * Exception thrown when JavaScript code throws an error. Wraps the JavaScript error value and its source location for
 * access from Java.
 */
public class JSException extends RuntimeException {
     ...
     * <p>
     * A {@code SourceLocation} is a pair of offsets, and offsets alone do not say which text they are offsets into.
     * While every located error came from the source the embedder had just handed to {@code eval}, that was implicit
     * and safe. It stopped being either when module linking began reporting failures that belong to a
     * <em>dependency</em>: those coordinates index a file the caller never passed in, so attaching them without saying
     * so would have described a position in the wrong text. The location was therefore dropped for those errors and the
     * coordinates were appended to the message instead, which left embedders parsing prose to find something the engine
     * already knew. Naming the source makes the pair complete, and the location can be carried in every case.
     */
     ...
     *
     * @return the source name, or {@code null} when the location belongs to the source the embedder itself supplied, or
     ...
     * <p>
     * Own data properties only. Reading {@code name} and {@code message} through {@link JSObject#get(PropertyKey)}
     * invokes accessors, so constructing this exception ran arbitrary user code at throw time — while an error was
     * already in flight — and that code could raise a second exception that displaced the one being reported.
     *
     * @param error
     *            the thrown value
     ...
     * <p>
     * {@code getOwnPropertyDescriptor} is virtual, and on a {@link com.caoccao.qjs4j.core.JSProxy} the override
     * <em>is</em> the {@code getOwnPropertyDescriptor} trap — so reading {@code name} and {@code message} that way
     * still re-entered guest code for a thrown Proxy, four times per reported exception.
     * {@code findDataPropertyForDiagnostics} is {@code final} and resolves to physical storage, which is the only read
     * in the engine that provably runs nothing.
     *
     * @param errorObj
     ...
     *            the property key
     * @param defaultValue
     *            the value to use when the property is absent, is an accessor, is not a string, or belongs to a Proxy
