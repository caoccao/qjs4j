...
public final class JSSyntaxError extends JSError {
    ...
    public JSSyntaxError(JSContext context, String message, SourceLocation sourceLocation) {
    ...
    public static JSValue create(JSContext context, JSValue... args) {
        ...
        JSSyntaxError jsSyntaxError = context.createJSSyntaxError(message);
        if (args.length > 1) {
            if (!JSError.installErrorCause(context, jsSyntaxError, args[1])) {
                return JSUndefined.INSTANCE;
            }
        }
