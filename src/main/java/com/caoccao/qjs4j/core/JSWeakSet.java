...
/**
 * Represents a JavaScript WeakSet object. Values must be objects and are weakly referenced. WeakSets are not
 * enumerable.
 * <p>
 * Membership lives on the member, in its {@link JSWeakEntryTable}, and names this set by identity — see
 * {@link JSWeakMap} and {@link JSWeakEntryTable}.
 */
public final class JSWeakSet extends JSObject {
     ...
     * <p>
     * The value must be an object or an <em>unregistered</em> symbol, exactly as {@code WeakSet.prototype.add} requires
     * — see {@link #isWeakSetValue(JSValue)}.
     *
     * @param value
     *            the member to add
     * @throws JSTypeErrorException
     *             when the value cannot be held weakly
     */
     ...
     * global registry keeps it alive for the realm's lifetime, so a membership keyed on one could never be collected.
     *
     * @param value
